...
the result is 'same' since both the department '1' and '2' have the same average salary with the company, 
which is 7000.
*/
select department_salary.pay_month, department_salary.department_id,
case 
    when department_salary.department_avg > company_salary.company_avg then 'higher'
...
(select e.department_id, avg(s.amount) as department_avg, date_format(s.pay_date, '%Y-%m') as pay_month
from salary s
join employee e on s.employee_id = e.employee_id
group by e.department_id, pay_month) as department_salary
